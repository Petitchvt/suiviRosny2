...
export default function PageAcheteur({ acheteur }) {
  const now = new Date();
  ...
  const labosNorms = acheteur.laboratoires.map(l =>
    l.trim().toUpperCase().replace(/\s*\d{4}\s*\/\/.*$/, '').replace(/\s*\*+$/, '').trim()
  );

  const entries = allEntries.filter(e => {
    const nomNorm = e.laboratoire.trim().toUpperCase().replace(/\s*\d{4}\s*\/\/.*$/, '').replace(/\s*\*+$/, '').trim();
    return labosNorms.some(l => l === nomNorm || nomNorm.startsWith(l) || l.startsWith(nomNorm));
  });
        ...
        <div className="mb-8 rounded-3xl px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="p-3 rounded-2xl shadow-inner"
              style={{ background: `${acheteur.couleur}22`, border: `2px solid ${acheteur.couleur}44` }}>
              <User className="w-6 h-6" style={{ color: acheteur.couleur }} />
            </div>
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-slate-800">{acheteur.nom}</h1>
              <p className="text-sm text-slate-500">{entries.length > 0 ? `${[...new Set(entries.map(e => e.laboratoire))].length} laboratoires • CA HT ${currentYear} vs ${currentYear - 1}` : 'Aucune donnée'}</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              style={{ background: `linear-gradient(135deg, ${acheteur.couleur}, ${acheteur.couleur}cc)`, boxShadow: `0 4px 15px ${acheteur.couleur}55` }}>
          ...
          <Tabs defaultValue="comparison">
            <div className="px-4 pt-4">
              <TabsList className="rounded-2xl p-1 w-full sm:w-auto"
                style={{ background: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.7)", backdropFilter: "blur(10px)" }}>
            ...
            <div className="p-4">
              <TabsContent value="comparison" className="mt-0">
                <LaboComparisonTable entries={entries} selectedMois={selectedMois} currentYear={currentYear} onEdit={handleEdit} onDelete={handleDelete} progression={progression} labosAcheteur={acheteur.laboratoires} onLaboClick={setSelectedLabo} prorataMode={prorataMode} onProrataChange={setProrataMode} />
              </TabsContent>
